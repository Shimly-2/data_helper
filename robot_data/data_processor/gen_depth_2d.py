...
@DATA_PROCESSER_REGISTRY.register("GenDepth2D")
class GenDepth2D(BaseDataProcessor):
    def __init__(
        self,
        workspace,
        table_path,
    ...
    def gen_per_depth2D(self, raw_img_list, task_path):
        ...
        for raw_img_path in tqdm(raw_img_list, colour="green", desc=f"PID[{os.getpid()}]: Process<{object_name}--{view}>"):
            ...
            depth[depth < 0] = 0
            save_img_path = raw_img_path.replace("rgb", "depth2D")
            plot_depth2D(depth, save_img_path)
