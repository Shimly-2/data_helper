...
def plot_depth2D(depth, depth_path):
    """绘制原始图像，原始图像与ref的差值，x和y的梯度，深度的2d和3d图"""
    plt.figure()
    plt.imshow(depth)
    plt.axis('off')
    plt.savefig(depth_path, bbox_inches='tight', pad_inches = -0.1)
    plt.close()

def plot_force2D(depth_sum, depth_min, depth_max, timestep, depth_path):
    plt.figure()
    plt.plot(timestep, depth_max, label='depth max', color='blue')
    plt.plot(timestep, depth_min, label='depth min', color='orange')
    plt.plot(timestep, depth_sum, label='depth sum', color='green')
...
@DATA_PROCESSER_REGISTRY.register("GenForce2D")
class GenForce2D(BaseDataProcessor):
    def __init__(
        self,
        workspace,
        table_path,
    ...
    def gen_per_force2D(self, raw_img_list, task_path, force_data_path):
        ...
        for raw_img_path in tqdm(raw_img_list, colour="green", desc=f"PID[{os.getpid()}]: Process<{object_name}--{view}>"):
            ...
            grad_img2 = matching_v2(raw_img, self.ref_blur, self.detector, self.table, self.blur_inverse)
            grad_img2[:, :, 0] = grad_img2[:, :, 0] * (1 - marker_mask) * self.red_mask
            grad_img2[:, :, 1] = grad_img2[:, :, 1] * (1 - marker_mask) * self.red_mask

            depth = fast_poisson(grad_img2[:, :, 0], grad_img2[:, :, 1])
            depth[depth < 0] = 0
            per_force_data["depth_sum"] = sum(map(sum, depth)) / 10000.0
            per_force_data["depth_max"] = np.max(depth)
            per_force_data["depth_min"] = np.min(depth)
            depth_sum.append(per_force_data["depth_sum"])
            depth_min.append(per_force_data["depth_min"])
            depth_max.append(per_force_data["depth_max"])
            timestep.append(idx)
            save_img_path = raw_img_path.replace("rgb", "force2D")
            plot_force2D(depth_sum, depth_min, depth_max, timestep, save_img_path)
    ...
    def process(self, meta, task_infos):
        ...
        self.ref_blur = cv2.GaussianBlur(ref_img.astype(np.float32), (3, 3), 0) + 1
        self.blur_inverse = 1 + ((np.mean(self.ref_blur) / self.ref_blur) - 1) * 2
        self.red_mask = (ref_img[:, :, 2] > 12).astype(np.uint8)
        if self.pool > 1:
            args_list = []
            for per_meta in meta:
                ...
                task_path = os.path.dirname(raw_img_list[0]).replace("rgb", "force2D")
                force_data_path = os.path.dirname(raw_img_list[0]).replace("rgb", "force_data")
                args_list.append((raw_img_list, task_path, force_data_path))
            results = self.multiprocess_run(self.gen_per_force2D, args_list)
        else:
            for idx, per_meta in enumerate(meta):
                ...
                results.append(
                    self.gen_per_force2D(raw_img_list, task_path, force_data_path))
