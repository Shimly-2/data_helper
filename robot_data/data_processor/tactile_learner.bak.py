...
@DATA_PROCESSER_REGISTRY.register("Tactile_Learner.bak")
class tactile_learnerbak(BaseDataProcessor):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.ref_img_path is not None:
        ...
        self.init_flow_matcher(self.ref_img)
    ...
    def gen_per_optical_flow_description(self, tactile_name, description_name):
        for imgs in self.imgs_list:
            path = imgs["gelsight_left"]["img_path"]
            self.logger.info(f"imgs_path {path}")
            ...
            for sensor in self.sensor_name:
                ...
                # imgs["gelsight_right"]["img_depth"] = self.nn.get_depthmap(self.net, imgs["gelsight_right"]["img_origin"], setting.MASK_MARKERS_FLAG)

                ### find marker masks
                imgs[sensor]["img_mask"] = marker_detection.find_marker(imgs[sensor]["img_origin"])
                # imgs["gelsight_right"]["img_mask"] = marker_detection.find_marker(imgs["gelsight_right"]["img_origin"])
                ### find marker centers
                imgs[sensor]["img_mc"] = marker_detection.marker_center(imgs[sensor]["img_mask"], imgs["gelsight_left"]["img_origin"])
                # imgs["gelsight_right"]["img_mc"] = marker_detection.marker_center(imgs["gelsight_right"]["img_mask"], imgs["gelsight_right"]["img_origin"])
            if self.use_calibrate == False:
                ### matching init
                self.flow_matcher.init(imgs["gelsight_left"]["img_mc"])
                ...
                # contact detection####################################################
                for sensor in self.sensor_name:
                    imgs[sensor]["img_contact"] = contact_detection_v2(imgs[sensor]["img_origin"], self.ref_img)
                    ...
                    ####################################################################
                        ...
                        os.makedirs(os.path.dirname(imgs[sensor]["img_path"].replace("rgb","flow")), exist_ok=True)
                        os.makedirs(os.path.dirname(imgs[sensor]["img_path"].replace("rgb","contact")), exist_ok=True)
                        os.makedirs(os.path.dirname(imgs[sensor]["img_path"].replace("rgb","uv_flow")), exist_ok=True)
                        cv2.imwrite(imgs[sensor]["img_path"].replace("rgb","flow"),imgs[sensor]["img_flow"])
                        cv2.imwrite(imgs[sensor]["img_path"].replace("rgb","contact"),imgs[sensor]["img_origin"])
                        cv2.imwrite(imgs[sensor]["img_path"].replace("rgb","uv_flow"),imgs[sensor]["img_uv_flow"])
                        save_path = imgs[sensor]["img_path"].replace("rgb","flow")
    ...
    def process(self, meta, task_infos):
        ...
        self.gen_imgs_list()
        self.gen_per_optical_flow_description("a","b")
    ...
    def process(self, meta, task_infos):
        ...
        if self.pool > 1:
            ...
            for per_meta in meta:
                ...
                task_path = os.path.dirname(raw_img_list[0]).replace("rgb", "depth2D")
                args_list.append((raw_img_list, task_path))
            results = self.multiprocess_run(self.gen_per_depth2D, args_list)
        else:
            for idx, per_meta in enumerate(meta):
                ...
                results.append(
                    self.gen_per_depth2D(raw_img_list, task_path))
        return meta, task_infos